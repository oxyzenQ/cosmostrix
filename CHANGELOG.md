...
Owner mandate per deepseek discussion: tighten the LOC cap from 1500 to a **hard limit of 800 lines per .rs file**, with a **soft target of 500 for new files** (see `src/RULES_LOC.md`). 38 files currently exceed 800 — migration is incremental, tracked via the `EXEMPT_BELOW_800` list in `scripts/check-rs-loc.sh`. Comprehensive A/B benchmark (10s release, visual + performance + quality metrics): avg_fps -0.544% (noise), frame_entropy -0.004% (noise), density_gini +0.005% (noise), color_transition_delta 0→0 (perfect), active_streams 23→23 (identical), alloc_calls 563→564 (negligible +1), 0 test regressions. **Visual quality confirmed unchanged across all dimensions.**
