...
- **Architecture**: the reveal math stays stateless in `types/msg_fill_style.rs` like the other six styles; the sparks live in a new dedicated 48-slot pool (`cloud/message_engrave.rs`, pre-allocated once, O(active)/frame, O(1) early-out when idle). A dedicated pass was required because the shared quantum pool renders in `apply_quantum_ripple` BEFORE `draw_message` — pool-shared sparks would be overdrawn by the overlay cells, invisible exactly where the engraving head lives. Sparks render at the END of `draw_message`, on top of the freshly burned-in text.
