...
Owner audit with deepseek flagged 11 files at 1200+ LOC. Codebase-wide refactor sweep extracted cohesive concerns into sibling modules to restore breathing room below the 1500-LOC cap. A/B benchmark (10s release, 3 commits): avg_fps 93961 → 94550 (+0.627%, within noise), peak_rss 4.25 → 4.24 MiB, heap_retained +752 bytes (negligible), 0 test regressions. Performance-neutral as promised.

- **`src/RULES.md` violation fixed (single-file policy)**: commit `e6f7cf8` (previous session) introduced `src/main_post_exit.rs` as a flat file at `src/` root — a direct violation of the single-file policy documented in `src/RULES.md` ("src/ root must contain exactly one .rs file: main.rs"). Moved to `src/output/post_exit.rs`, following the established pattern (`output/verbose.rs`, `output/report.rs`, `output/ux.rs`, `output/message.rs` are all output-printing siblings). Git detected the move as a rename (96% similarity), preserving file history. Call site updated from `main_post_exit::print_post_exit_verbose(...)` to `output::post_exit::print_post_exit_verbose(...)`. `src/` root now contains ONLY `main.rs` + `RULES.md`.
- **`bench/mod.rs` extract `compute_peak_fps` to `peak_fps.rs`**: the peak-FPS computation (pure function: takes `&[f64]` + `usize`, returns `f64`) was extracted to `src/bench/peak_fps.rs`. Re-exported via `pub(crate) use peak_fps::compute_peak_fps;` so all call sites (including `use super::*` glob in `bench_tests.rs`) resolve unchanged. LOC: bench/mod.rs 1330 → 1262 (68 lines saved).
- **`hud/mod.rs` extract color helpers to `colors.rs`**: 2 pure color functions (163 lines incl. docs) extracted to `src/interactive/hud/colors.rs`: `compute_chroma_gradient_18` (18-stop chroma gradient via `interpolate_palette_color` + `brighten_color` floor) + `brighten_color` (HSV value scaling with `TARGET_V=200`). Re-exported via `pub(crate) use colors::{brighten_color, compute_chroma_gradient_18};`. LOC: hud/mod.rs 1298 → 1144 (154 lines saved — biggest single-file win in this sweep).
- **Prior session extractions (re-listed for completeness)**: `event_loop.rs` intro sequence → `event_loop_intro.rs` (commit `e3189ea`, 74 lines); `cloud/mod.rs` `interpolate_palette_color` → `cloud/palette_blend.rs` (commit `b59dc7d`, 69 lines); `event_loop.rs` `sync_base_cfg_with_runtime_scene` → `event_loop_scene_sync.rs` (commit `80c2cc2`, 31 lines); `main.rs` post-exit verbose dump → `output/post_exit.rs` (commit `e6f7cf8`, 28 lines).
