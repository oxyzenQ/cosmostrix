...
- **`notify` 6→7 upgrade (compile-time win)**: bumped `notify` from `>=6.1, <7` to `>=7, <8`. This eliminates the duplicate `mio` crate (v0.8.11 from notify 6 + v1.2.2 from crossterm → now only v1.2.2). `bitflags` v1.3.2 is still pulled by `inotify v0.10.2` (a notify 7.0.0 transitive dep that hasn't migrated to bitflags 2 yet — expected to resolve in notify 7.1+). Net compile-time savings: ~0.7s (one fewer `mio` build) plus reduced incremental cache pressure. The API migration was transparent — `RecommendedWatcher::new` + `Watcher::watch` signatures are unchanged in 7.x for the cosmostrix call sites. All 1723 tests pass with the new version. Closes Priority 1 recommendation from `docs/audits/DEPS_AUDIT_v50.0.0-beta.7.md`.
- **`ambient-palette-lock` config key (Option C — config-tunable Crystal Dragon drift suppression)**: by default, ambient fires lock the palette to prevent Crystal Dragon drift from "fighting" the ambient schedule. The owner can now set `ambient-palette-lock = false` to allow Crystal Dragon drift to run **automatically** while ambient is on — no need to press `c`/`C` to unlock. Default `true` (current behavior, no breaking change). Live-reloadable. When `false`, `ambient-snapback-secs` becomes a no-op (drift already overrides the ambient palette). Closes the owner's bug report: "crystal dragon is not working while ambient on, only works when I press c/C". 3 lock tests added: `live_reload_ambient_palette_lock_false_from_config`, `live_reload_ambient_palette_lock_defaults_none_when_unset`, `live_reload_ambient_palette_lock_true_is_valid`.
