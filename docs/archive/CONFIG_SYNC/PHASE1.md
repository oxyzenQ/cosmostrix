...
- Type asymmetry: `speed = 15.5` accepted in `adaptive-custom`, rejected in
...
- Rewrite doc comment to actual 5-level chain:
 1. clap defaults
...
- `src/interactive/event_loop.rs:249-257`: parses `custom_time_map` from
 `cfg_map` without check `args.atmosphere_mode == Disabled`
...
**undocumented in user-facing surface**.
...
- Warning which should be error (e.g., `--screensaver --intro none`)
- Missing warnings for footguns (e.g., `--benchmark --intro logo`)
- Config keys which typo'd silently ignored vs unknown-key tracking
...
- Path traversal in `--config` / `--dump-config` — `safepath.rs` (audit)
- Race conditions in live config reload — `live_config.rs` (Phase 4 read)
- Mutex poison — Gap #11
