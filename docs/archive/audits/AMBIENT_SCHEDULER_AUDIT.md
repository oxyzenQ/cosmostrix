...
**Root cause** (`src/engine/crystal_dragon_engine/ambient_scheduler.rs:217-231`):
...
### 2.1 Patch A — Day-boundary refire in `src/engine/crystal_dragon_engine/ambient_scheduler.rs`

Add a `current_yday()` helper in `src/engine/crystal_dragon_engine/ambient.rs` (mirrors
...
1. **`src/engine/crystal_dragon_engine/ambient_scheduler.rs` unit tests**:
