...
# Changelog — Pre-v13 Archive

This file is a verbatim copy of every CHANGELOG entry older than `v13.0.0`.
It was split out of the live `CHANGELOG.md` as part of Dragon Hunt v2
Phase 3 (Tier B item 23) to keep the live changelog focused on currently-
supported versions.

## Ordering

Entries are in the same reverse-chronological order as the parent
`CHANGELOG.md`. The top of this file is `v11.1.0` (the entry immediately
below `v13.0.0` in the live file); the bottom is `v2.0.0` (the oldest
entry in the project's history).

## Tripwire note

`v4.0.0` and `v3.9.0` also appear in the live `CHANGELOG.md` because
`src/docs_tests/metadata.rs` enforces their presence via
`include_str!("../../CHANGELOG.md")`. The two copies are byte-identical.
