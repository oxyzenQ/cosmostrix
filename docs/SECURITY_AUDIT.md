...
Documented **"no new unsafe in renderer/core paths"** policy (`docs/SIMD_FEASIBILITY.md` §1, §2.3; `docs/RULES.md`). Every `unsafe` site is FFI into `libc`/Mach and carries a `// SAFETY:` comment. **No `unsafe` in the renderer hot path** (verified by `diagnostics/info.rs:303`). **Total**: 15 distinct sites + 1 `unsafe fn` definition.
