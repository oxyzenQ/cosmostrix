...
`docs/archive/audits/UNSAFE_SOUNDNESS_AUDIT.md` (2026-08-04) confirms manual + Miri review of all 15 sites: **0 unsound, 0 fixes needed**. **Custom allocator note**: `TraceAlloc` (`src/diagnostics/alloc_trace.rs:46`) is wired as the global allocator (`src/main.rs:42-43`) and is always active in production builds. Thin wrapper around `std::alloc::System` that only adds updates to a const-initialized thread-local counter slot — no allocation, no lock, no synchronization, no I/O, no syscalls beyond what `System` already does (NIGHT-perf-2 2026-09-25: the atomic process-global counters became per-thread counters so a benchmark window measures the measuring thread's own allocations, not the whole test process — see the FreeBSD CI incident note in `docs/BENCHMARKING.md`; the unsafe surface and the no-reentrancy argument are unchanged). Counters only read by the benchmark subsystem; if no benchmark runs, they accumulate harmlessly. Not a security concern.
...
**Symlink scope note** (NIGHT-ultimate-1, 2026-09-24): the whitelist is lexical by design — it does not `canonicalize()` or resolve symlinks. A symlink planted *inside* a whitelisted directory (e.g. `~/.config/cosmostrix/theme.toml -> ~/.ssh/id_rsa`) therefore passes the prefix check, and cosmostrix will read through it. This is an accepted residual for a user-run, single-user CLI tool: planting the symlink already requires write access to the user's own config directory, at which point the attacker has strictly stronger primitives (the same access allows editing the user's shell rc files). Defending at read time would add TOCTOU-prone `canonicalize` races without raising the actual bar. Every config-read entry point (`--config`, `--testconf`, `--dump-config`, bench baselines) routes through this validator; reads are size-capped at 1 MiB (`read_config_capped`) and `.toml`-only, which limits symlink-based reads of non-config targets to parsing failures in practice.
