...
- **`ipc`** — instructions per cycle. >2.0 = healthy; >3.0 = excellent. v50 PGO v3: 2.52–2.62 (Zen 3). Below 2.0 suggests the binary was built without AVX2 (`pro-linux-amd64-v3-gnu`) or AVX-512 (`pro-linux-amd64-v4-gnu`).
- **`branch_mispredict_rate`** — branch predictor failure rate. <2% = BOLT lookup tables working. v30: 0.57–2.41%. High mispredict rates suggest BOLT was not applied (use `pro-linux-amd64-v3-gnu`/`pro-linux-amd64-v4-gnu` profile or `nitro-pgo`).
