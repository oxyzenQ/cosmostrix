...
```bash
./scripts/version-to.sh 25.0.0-alpha.1
git diff
git commit -m "chore: bump version to v25.0.0-alpha.1"
git tag v25.0.0-alpha.1
git push origin main v25.0.0-alpha.1
```
...
It skips changelog headings (e.g. `### v25.0.0-alpha.1`) to preserve historical release notes, and audits workflow files for hardcoded versions (workflows should derive versions dynamically from `GITHUB_REF_NAME`).

Verify the current version without making changes:

```bash
./scripts/version-to.sh --check 25.0.0-alpha.1
```
