...
1. **Overlay UI** — `interactive/hud/colors.rs::brighten_color` (HSV-value scaling for HUD readability), `intro_style::lerp_rgb` (linear-sRGB blend for intro particle fade; was `interactive/intro.rs` pre-v52). Different math semantics; migrating would break regression tests and produce ±1/channel visible artifacts.
