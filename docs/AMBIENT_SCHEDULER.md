...
| Question | Answer |
|---|---|
| How often does crystal-dragon act? | Drift cadence = **`crystal-dragon-secs`** (default **60s**, range 0.0..=86400.0 — CLI `--crystal-dragon-secs`, config key, live-reload). The min-dwell anti-flicker floor is **min(60s, cadence)**: at the default (or slower) palette flips cap at one per minute; a faster explicit cadence is honored as-is (S-master-HUNT-3 — the knob is real below 60s too). Cadence verified live: a 6s cadence drifted at ~6s. S-master-HUNT-6 (2026-09-03): the drift DECISION is gated on the poll boundary itself (dice no longer roll every frame), so a SLOWER-than-60s cadence is now honored verbatim (10m = one drift decision per 10 minutes) — and enabling crystal-dragon mid-session via a config edit ARMS the clock (first drift owed one full interval later, no millisecond burst). |
...
removed; custom scenes pick rain style via the block's `rain` field (NIGHT-research-5).
...
[verbose] [HH:MM]   snapback_secs:  10.0s (from config — drift visible for 10.0s before ambient reverts)
...
```text
[verbose] [HH:MM]   snapback_secs:  30.0s (default (unset in config) — drift visible for 30.0s before ambient reverts)
```
...
```text
[verbose] [HH:MM]   snapback_secs:  10.0s (config) (was 30.0s)
...
|--------|----------------|
| `src/engine/crystal_dragon_engine/ambient/mod.rs` | Parser, `AmbientEntry` / `AmbientSchedule` structs (`AmbientEntry` is just `{hour, minute, scene}`), `current_phase` / `next_phase` / `seconds_to_next_phase` helpers, strict validation (`validate_ambient_entries`), wall-clock helpers (`current_minute_of_day` for event-gated callers; `AmbientClockSnapshot::now()` — one `local_tm()` read for minute + second + yday — for the scheduler thread, NIGHT-hunter-12) |
...
| `src/engine/cosmic_dragon_engine/cloud/scene_runtime.rs` | `Cloud::apply_ambient_entry` — delegates to `apply_scene_runtime_with_cfg`, which handles both built-in scenes (fast path) and custom scenes (looks up the `[scene-custom.<name>]` block and applies its complete self-contained field layer — v80.0.0-beta.2: no `base-scene` inheritance; NIGHT-research-5: rain style picked via block's `rain` field, falls back to Glyph) |
...
| `src/scene_custom/mod.rs` | `UserProfile` struct (seven scene-family dimensions), `ambient_scene_fps` (built-in default or block field), `validate_scene_custom_completeness` (all seven required), `resolve_rain_style` (custom scenes pick rain via block's `rain` field — NIGHT-research-5) |
...
| **DST spring-forward** (2:00 AM -> 3:00 AM) | `AmbientClockSnapshot::now()` returns wall-clock local time. Entries in the skipped hour (02:00–02:59) are never fired. Acceptable. |
