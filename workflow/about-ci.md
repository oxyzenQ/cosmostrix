...
git commit -am "release: 3.9.0"

# 3) Create an annotated tag
git tag -a v3.9.0 -m v3.9.0
...
./version-to.sh 3.9.0
git diff
git commit -m "chore: bump version to v3.9.0"
git tag v3.9.0
...
It skips changelog headings (e.g. `### v3.9.0`) to preserve historical release notes.

Verify the current version without making changes:

```bash
./version-to.sh --check 3.9.0
```
