...
git commit -am "release: 3.8.0"

# 3) Create an annotated tag
git tag -a v3.8.0 -m v3.8.0
...
It skips changelog headings (e.g. `### v3.8.0`) to preserve historical release notes.

Verify the current version without making changes:

```bash
./version-to.sh --check 3.8.0
```
