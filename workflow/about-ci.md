...
./version-to.sh 3.6.0
git diff
git commit -m "chore: bump version to v3.6.0"
git tag v3.6.0
...
It skips changelog headings (e.g. `### v3.6.0`) to preserve historical release notes.

Verify the current version without making changes:

```bash
./version-to.sh --check 3.6.0
```
