...
- **Custom color palettes** — `[colors-custom.<name>]` blocks define 2–9-stop TrueColor palettes (complete blocks: `bg` AND `rain` — missing either is a hard error, and more than 9 stops is rejected since the OKLab engine resamples every palette to 9 perceptual samples); referenced via `--colors <name>`, the `color = "<name>"` config key (custom names accepted, same as charset — v80.0.0-beta.2), or from scenes.
