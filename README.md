...
Press `x` while running to cycle through all built-in scenes (cinematic -> monolith -> matrix -> vortex -> flux -> lorenz -> cosmic_dragon -> physarum -> sorgonemous_intrascals -> aeolian -> solar_flare -> classic -> … -> curiosity, then back to cinematic).
