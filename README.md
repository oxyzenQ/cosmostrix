...
- **User-defined custom scenes** — `[scene-custom.<name>]` blocks in config, applied via `--scene <name>` (auto-promotes custom names) or `--scene-custom <name>`, or the `scene = "<name>"` config key. v80.0.0-beta.2 (S-master-LOGIC-3) + NIGHT-research-5: a block is a COMPLETE seven-dimension profile — `rain`, `color`/`colors-custom`, `charset`/`charset-custom`, `fps`, `speed`, `density`, `glitch-level`, ALL required (incomplete blocks are rejected); `rain` picks any of the ten rain styles by canonical label (`glyph`, `monolith`, `vortex`, `flux`, `lorenz`, `dragon`, `physarum`, `black_hole`, `aeolian`, `aurora` — e.g. `rain = "lorenz"`); `base-scene` inheritance is REMOVED (the block's `rain` field is the only non-glyph source); all three selection surfaces (CLI flags, config key, live-reload edit) accept custom names uniformly, and at runtime a present block field overrides the locked CLI startup value (config wins — the CLI lock is only the fallback).
...
- `aurora` — the invented polar veil (NIGHT-special-3, the second original-math style): vertical glyph curtains hang from the top edge, each carried by an invisible ray bead — one bead per ~6 columns — and the whole lattice drifts on a slow global wind while the beads repel each other into an organic spread. The curtains breathe between two disjoint depth bands (a quantized substorm anchor: shallow curtains hang at 24-34% of the screen height, deep ones at 46-60%, flipping on rolled dwell intervals), so the veil reads as layered altitudes with a wavy fold silhouette, never a uniform mush. The rain is the precipitation: glyph drops fall from the top edge, funnel toward the brightest nearby fringe (a glow-weighted seek — bright curtains collect the weather), and are absorbed AT the emission depth, depositing their kinetic charge into the fringe glow — the light in this sky is literally where the rain has been landing. A charged fringe climbs the brightness ladder (Mid baseline, Hot charged, Core for a short flare window after a fresh landing) and spills into its flanking columns; the curtain body above fades exponentially into the dark (a diffuse top edge, a fabric — cells keep their glyphs, shimmering rarely, while the fringe flickers hard). Every state variable is bounded by construction (velocity/position/depth clamps, hard-capped glow, lane-bounded pool with terminal velocity and a lifetime backstop) — LTS without a proof obligation; absorption is a state test, not a crossing test, so no tunneling at any dt. Aurora palette (557.7nm green — the oxygen line the real curtains burn on), greek glyphs. `cosmostrix --scene aurora`
...
Press `x` while running to cycle through all built-in scenes (cinematic -> monolith -> matrix -> vortex -> flux -> lorenz -> cosmic_dragon -> physarum -> sorgonemous_intrascals -> aeolian -> aurora -> classic -> … -> curiosity, then back to cinematic).
