...
// ── P4: stuck-cell sweep tests ───────────────────────────────────────────────
//
// The sweep is a debug-only watchdog that catches cells the phosphor system
// misses. These tests verify the gating logic and the cell-clearing behavior
// using a synthesized stuck cell (one that has fg, current_gen, no phosphor
// energy, and is not covered by any droplet).

use crate::cell::Cell;
use crate::cloud::Cloud;
use crate::constants::{STUCK_CELL_MAX_PER_SWEEP, STUCK_CELL_SWEEP_INTERVAL_FRAMES};

/// Helper: build a cloud with component timing enabled so the sweep can fire.
