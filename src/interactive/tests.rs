...
mod cases {
    ...
    // --- 'r' key: rain preset cycling tests ---

    #[test]
    fn lowercase_r_cycles_rain_preset_from_cinematic_to_organic() {
        let mut cloud = make_test_cloud();
        let mut frame = Frame::new(cloud.cols, cloud.lines, cloud.palette.bg);
        let mut charset_preset = String::from("binary");

        // Default preset is Cinematic
        assert_eq!(
            cloud.rain_preset(),
            crate::rain_preset::RainPreset::Cinematic,
            "default rain preset should be Cinematic"
        );
        ...
        assert_eq!(
            cloud.rain_preset(),
            crate::rain_preset::RainPreset::Organic,
            "pressing 'r' once should switch to Organic"
        );
    }

    #[test]
    fn pressing_r_twice_cycles_back_to_cinematic() {
        let mut cloud = make_test_cloud();
        let mut frame = Frame::new(cloud.cols, cloud.lines, cloud.palette.bg);
        let mut charset_preset = String::from("binary");

        // First press: Cinematic → Organic
        call_handle_keybinding(
            &mut cloud,
            &mut frame,
            &key('r'),
        ...
        assert_eq!(cloud.rain_preset(), crate::rain_preset::RainPreset::Organic);

        // Second press: Organic → Cinematic
        call_handle_keybinding(
            &mut cloud,
            &mut frame,
            &key('r'),
        ...
        assert_eq!(
            cloud.rain_preset(),
            crate::rain_preset::RainPreset::Cinematic,
            "pressing 'r' twice should return to Cinematic"
        );
    }

    #[test]
    fn pressing_r_applies_preset_short_pct() {
        let mut cloud = make_test_cloud();
        let mut frame = Frame::new(cloud.cols, cloud.lines, cloud.palette.bg);
        let mut charset_preset = String::from("binary");

        // Organic preset should set short_pct to 0.7
        call_handle_keybinding(
            &mut cloud,
            &mut frame,
            &key('r'),
        ...
        assert!(
            (cloud.short_pct - 0.7).abs() < 1e-6,
            "Organic preset should set short_pct=0.7, got {}",
            cloud.short_pct
        );

        // Press 'r' again to go back to Cinematic (short_pct=0.5)
        call_handle_keybinding(
            &mut cloud,
            &mut frame,
            &key('r'),
        ...
        assert!(
            (cloud.short_pct - 0.5).abs() < 1e-6,
            "Cinematic preset should set short_pct=0.5, got {}",
            cloud.short_pct
        );
    }

    #[test]
    fn pressing_r_forces_redraw_for_immediate_pivot() {
        let mut cloud = make_test_cloud();
        let mut frame = Frame::new(cloud.cols, cloud.lines, cloud.palette.bg);
        let mut charset_preset = String::from("binary");
        // Clear flags set by make_test_cloud so we start clean.
        cloud.clear_redraw_flags_for_test();
        ...
        assert!(
            cloud.is_force_draw_everything(),
            "pressing 'r' should force a full redraw so the phosphor decay change is visible immediately"
        );
    }
}
