...
mod cases {
    ...
    // (verified by `v35_cloud_ambient_flags_default_false` above). On the first
    // live reload, the event loop's empty-schedule handler checked this flag to
    // decide whether to preserve the user's state or re-apply scene defaults.
    // Since it was `false`, the handler called `cloud.apply_scene_runtime("cinematic")`,
    // which overrides:
    ...
    fn v50_apply_scene_runtime_cinematic_overrides_explicit_user_color() {
        let mut cloud = Cloud::new(
            ...
            false,
            true,
            crate::runtime::ColorScheme::NeonGreen, // user's explicit color
            crate::rain_style::RainStyle::Glyph,
        );
        cloud.init_chars(vec!['0', '1']);
        cloud.reset(20, 10);
        ...
        assert_eq!(cloud.color_scheme, crate::runtime::ColorScheme::NeonGreen);
        ...
        assert_ne!(
            cloud.color_scheme,
            crate::runtime::ColorScheme::NeonGreen,
