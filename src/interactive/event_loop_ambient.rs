...
/// guard below share one budget via `ctx.ambient.last_ground_truth_check`).
///
...
pub(crate) fn poll_ambient_events(ctx: &mut LoopCtx) -> Option<f64> {
    ...
    if !ctx.ambient.last_schedule.entries.is_empty() {
        while let Ok(entry) = ctx.ambient.handle.rx.try_recv() {
            if !ctx
                .ambient
                .last_schedule
                .entries
        ...
        while ctx.ambient.handle.rx.try_recv().is_ok() {} // drain stale
    }
    ...
    if last_ambient_entry.is_some()
        && ctx.ambient.last_ground_truth_check.elapsed()
            >= std::time::Duration::from_secs(GROUND_TRUTH_MIN_INTERVAL_SECS)
    {
        ctx.ambient.last_ground_truth_check = Instant::now();
        if let Some(ref path) = ctx.ambient.path_for_ground_truth {
            if let Ok(c) = crate::config_io::read_config_capped(path) {
                    ...
                    .is_empty()
                {
                    ...
                    if revert_ambient_owned_scene(ctx) {
                        ...
                        // the scene family.
                        fps_intent = Some(ctx.config.startup.target_fps);
                    }
                    ctx.ambient.last_schedule.entries.clear();
    ...
    if let Some(entry) = last_ambient_entry {
        if ctx.ambient.last_applied_entry.as_ref() == Some(&entry)
            && !ctx.cloud.user_override_since_ambient
        {
            // Duplicate — already applied.
        } else if ctx.cloud.user_override_since_ambient {
            ...
            if ctx.ambient.snapback_killed {
    ...
    let _ab06_sked_len = ctx.ambient.last_schedule.entries.len() as u64;
    let _ab06_last_applied = ctx.ambient.last_applied_entry.is_some();
    super::ambient_diag_snapback_guard(_ab06_sked_len, _ab06_last_applied);
    let guard_budget_ok = ctx.ambient.last_ground_truth_check.elapsed()
        >= std::time::Duration::from_secs(GROUND_TRUTH_MIN_INTERVAL_SECS);
    let ground_truth_ambient_empty = if !ctx.ambient.snapback_killed
        && guard_budget_ok
        && _ab06_sked_len > 0
        && _ab06_last_applied
    {
    ...
    if !ctx.ambient.snapback_killed
        ...
        && super::input::try_auto_snapback(ctx)
    {
        ctx.term
            .set_color_cache(ColorCache::new(&ctx.cloud.palette));
        ctx.frame = Frame::new(ctx.w, ctx.h, ctx.cloud.palette.bg);
        ...
        // is in `ctx.ambient.last_applied_entry` (updated by the callee).
        if let Some(entry) = ctx.ambient.last_applied_entry.as_ref() {
            let cfg_map = ctx.config.last_applied_map.clone().unwrap_or_default();
            fps_intent = crate::scene_custom::ambient_scene_fps(&entry.scene, &cfg_map);
        }
    }
...
/// tail block is ownership-gated). The ctx.cloud-level restore here mirrors
...
/// `ctx.config.startup`. `ctx.config.last_applied_map` is no longer needed — nothing is
/// re-derived from the config.
///
/// No-op when the scene is not ambient-owned or already matches the
/// startup scene.
fn revert_ambient_owned_scene(ctx: &mut LoopCtx) -> bool {
    let ambient_owned = !ctx.cloud.user_override_since_ambient
        && ctx.scene.scene_name != ctx.config.startup.scene_name
        && ctx
            .ambient
            .last_applied_entry
            .as_ref()
            .is_some_and(|e| e.scene == ctx.scene.scene_name);
    if !ambient_owned {
        return false;
    }
    crate::lr_trace!(
        ...
        ctx.config.startup.scene_name
    );
    ctx.scene.scene_name = ctx.config.startup.scene_name.clone();
    ctx.scene.scene_generation = ctx.scene.scene_generation.wrapping_add(1);
    ctx.cloud
        .set_scene_label(ctx.config.startup.scene_name.as_str());
    ...
    if ctx.cloud.rain_style != ctx.config.startup.rain_style {
        ctx.cloud
            .transition_rain_style(ctx.config.startup.rain_style);
    }
    ...
    if let Some(ref locked_palette) = ctx.config.startup.custom_palette {
        ctx.cloud.set_palette(
            ctx.config.startup.custom_palette_name.as_deref(),
            locked_palette.clone(),
        );
    } else {
        ctx.cloud.set_color_scheme(ctx.config.startup.color_scheme);
    }
    ...
    if ctx.scene.charset_preset != ctx.config.startup.charset_preset {
        ctx.cloud.transition_chars(ctx.config.startup.chars.clone());
    }
    ctx.scene.charset_preset = ctx.config.startup.charset_preset.clone();

    // Speed / density / glitch: snapshot values (CLI flags shadowed the
    // scene/block fields at startup — re-derivation would lose that).
    ctx.cloud.set_chars_per_sec(ctx.config.startup.speed);
    ctx.cloud.set_droplet_density(ctx.config.startup.density);
    ctx.cloud
        .apply_glitch_level_runtime(ctx.config.startup.glitch_level);

    // Clean redraw bookkeeping (same flags the scene-runtime applies set).
    ctx.cloud.semantic_invalidate = true;
    ctx.cloud.force_draw_everything = true;
