...
pub(super) fn run_intro_sequence(
    ...
    // NIGHT-hunt-32 (the hunt-31 follow-up revert): the intro handover
    // keeps its documented contract verbatim — full clear + force
    // redraw — and every style then plays its own full birth sequence
    // from the first post-intro frame. The hunt-31 fast-forward that
    // skipped the black hole's stellar-collapse dwell (the seed dot +
    // collapse cross) read as a missing genesis visual to the owner:
    // the startup is meant to carry the full formation cinematic, not
    // jump straight to the bloom. Reverted whole — no style's birth
    // choreography is fast-forwarded at the handover anymore.
    Ok(())
}
