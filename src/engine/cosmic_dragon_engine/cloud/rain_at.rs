...
//!
//! Extracted from `cloud/rain.rs` to keep that file under the 800-LOC cap.
...
impl super::Cloud {
    pub fn rain_at(&mut self, frame: &mut Frame, now: Instant) {
        ...
        // (v80.0.0-beta.1: +0.20 shift, same band edges) to shed more spawn load. This
        ...
        //
        // v50.0.0-beta.6 Option D + v80.0.0-beta.1 masterclass: uses the shared
        // `compute_spawn_scale()` function (single source of truth). The
        // HUD's `dsty:` metric uses the same function to display the
        // effective density. v80.0.0-beta.1: the user's configured density
        ...
        if self.semantic_invalidate {
            ...
            if self.rain_style.is_droplet_family() {
                ...
                if matches!(self.rain_style, RainStyle::Monolith) {
                    ...
                    // invalidation, mirroring monolith/vortex/flux.
            ...
            if use_sim_cap {
                for i in 0..self.droplets.len() {
                    ...
                    if died {
                        ...
                        // The ripple impact hook (spawn a ring + splash on
                        // droplet death at the water line) was removed with
                        // the ripple style. Flux and Lorenz have no droplet
                        // pool, so this branch is Glyph-only now and
                        // droplet death is plain cleanup.
                        continue;
                    }
        ...
        if matches!(self.rain_style, RainStyle::Monolith) {
            ...
            // The post-droplet ripple-surface draw pass was removed with
            // the ripple style. Flux and Lorenz are structured family
            // styles with their own draw branches above (mirrors
            // vortex); they never reach this Glyph-only path.
        }
