...
pub(super) fn ensure_test_config_dir_allowed() {
...
pub(super) fn args_with_config(config: &str, cli: &[&str]) -> Args {
...
pub(super) fn args_from_cli(cli: &[&str]) -> Args {
...
pub(super) fn args_from_cli_result(cli: &[&str]) -> Result<Args, String> {
    if cli.contains(&"--config") {
        ensure_test_config_dir_allowed();
        let mut argv = vec!["cosmostrix"];
        argv.extend_from_slice(cli);
...
// v50.0.0-beta.7 LOC refactor: --async-mode tests extracted to async_mode.rs;
// strict-mode tests extracted to strict_mode.rs;
// v50 message/message-border/msg-mode tests extracted to v50_message_keys.rs;
