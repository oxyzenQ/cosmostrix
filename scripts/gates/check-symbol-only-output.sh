...
#   src/**/*.rs, test/**/*.rs, build.rs, scripts/**/*.sh,
#   scripts/**/*.py,
#   benchmark/*.sh, .github/workflows/*.yml, pgo-runner/src/**/*.rs
#   (pgo-runner is a separate binary that also prints diagnostics;
#    test/**/*.rs joined the scope in NIGHT-hunter-5: the mirrored
#    test tree from NIGHT-hunter-1 carries the same output contract)
#
...
#   scripts/gates/check-symbol-only-output.sh  this file: the denylist itself
#   src/output/message.rs                sanitizer test INPUT: needs a real
#                                        emoji as data to verify replacement
#
# Companion tools:
#   scripts/audit/emoji-audit.py  — doc-prose sweep (manual, non-blocking)
#   this gate               — output surface (hard fail, gate-keepers + CI)
#
# Usage: bash scripts/gates/check-symbol-only-output.sh
...
# ── Exemptions ──────────────────────────────────────────────────────────────
EXEMPT=(
	"scripts/gates/check-symbol-only-output.sh"
...
# NIGHT-hunter-5: the mirrored test/ tree (NIGHT-hunter-1) holds real
# source files with the same output-surface contract — scan it too.
if [ -d test ]; then
	while IFS= read -r -d '' file; do
		check_file "$file"
	done < <(find test -name '*.rs' -not -path '*/target/*' -print0 2>/dev/null)
fi
