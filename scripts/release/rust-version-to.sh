...
# `scripts/gates/check-rust-version-sync.sh` verifier.
#
# Mirrors the contract of `scripts/release/version-to.sh` (project version
...
#
# USAGE:
#   ./scripts/release/rust-version-to.sh <X.Y.Z>           Bump to X.Y.Z (e.g. 1.99.0)
#   ./scripts/release/rust-version-to.sh --check <X.Y.Z>   Verify everything is at X.Y.Z
#   ./scripts/release/rust-version-to.sh --help            Show this help
#   ./scripts/release/rust-version-to.sh --allow-dirty <X.Y.Z>  Bump even on dirty tree
#
# EXAMPLES:
#   ./scripts/release/rust-version-to.sh 1.99.0           # Bump to 1.99.0
#   ./scripts/release/rust-version-to.sh --check 1.98.0   # Verify current pin
#
# Safety:
...
USAGE:
    ./scripts/release/rust-version-to.sh <X.Y.Z>                  Bump to X.Y.Z
    ./scripts/release/rust-version-to.sh --check <X.Y.Z>          Verify pin is X.Y.Z
    ./scripts/release/rust-version-to.sh --allow-dirty <X.Y.Z>   Bump even on dirty tree
    ./scripts/release/rust-version-to.sh --help                  Show this help
...
audit_docs() {
	...
	if [[ "${stale_count}" -gt 0 ]]; then
		...
		log_warn "  ./scripts/release/rust-version-to.sh --check ${new_full}"
	else
...
print_summary() {
	...
	echo "  ./scripts/build/build.sh check-all          # full gatekeeper"
	echo "  git diff"
...
run_check_mode() {
	...
	if bash "${SYNC_CHECK}" >/dev/null 2>&1; then
		...
		log_err "  ./scripts/release/rust-version-to.sh ${target_full}"
