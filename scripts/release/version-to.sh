...
#
# USAGE:
#   ./scripts/release/version-to.sh <VERSION>              Bump to VERSION (stable or pre-release)
#   ./scripts/release/version-to.sh --check <VERSION>     Verify version is VERSION (no changes)
#   ./scripts/release/version-to.sh --help                Show this help
#
# EXAMPLES:
#   ./scripts/release/version-to.sh v50.0.0                # Stable release
#   ./scripts/release/version-to.sh v50.0.0-alpha.2        # Pre-release
#
# Safety:
...
USAGE:
    ./scripts/release/version-to.sh <VERSION>              Bump to VERSION (stable or pre-release)
    ./scripts/release/version-to.sh --check <VERSION>      Verify version is VERSION
    ./scripts/release/version-to.sh --help                 Show this help
...
validate_version() {
	...
	# users naturally type `./scripts/release/version-to.sh v18.0.0`. Internally
...
verify_version() {
	...
	if [[ -f "${README}" ]]; then
		...
		if grep -qF "${expected_tag}" "${README}"; then
			...
			log_err "  Run './scripts/release/version-to.sh v${expected_ver}' to sync all active files"
			((errors++))
		fi
	fi
...
print_summary() {
	...
	echo "  cargo pro-linux-amd64-v3-gnu"
	echo "  target/x86_64-unknown-linux-gnu/pro-linux-amd64-v3-gnu/cosmostrix --doctor"
	echo "  git diff"
