...
BIN = (
    ...
    if "--bin" in sys.argv
    else "./target/release/cosmostrix"
)
...
def test(
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".toml", delete=False, dir=config_dir
    ) as f:
...
def test_colors_blocks():
    ...
    for i in range(105):
        ...
        lines.append('rain = "#000000, #ffffff"')
        lines.append("")
    config = "\n".join(lines) + "\n"
...
def test_unknown_fields():
    ...
    test(
        "charset_unknown_field_color_rejected",
        config,
        expect_pass=False,
    ...
    test(
        "colors_unknown_field_speed_rejected",
        config,
        expect_pass=False,
        expect_error_contains=["unknown"],
    )
    ...
    test(
        "scene_unknown_field_intro_rejected",
        config,
        expect_pass=False,
        expect_error_contains=["unknown"],
    )
