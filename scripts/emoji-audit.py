...
Exemptions (file-level, each justified -- keep this list SHORT):
  scripts/check-symbol-only-output.sh  the denylist itself (literal glyphs)
...
Usage:
  python3 scripts/emoji-audit.py          strict scan (gate mode)
  python3 scripts/emoji-audit.py --fix    replace known emoji, re-scan
"""
...
EXEMPT_FILES = {
    "scripts/check-symbol-only-output.sh": "embeds the denylist glyphs",
