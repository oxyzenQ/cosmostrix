...
# NIGHT-hunt-35: resolve the binary with a release fallback. The script
# previously hard-pinned ./target/debug/cosmostrix — on a release-only
# checkout every case failed with "expected tip not found" because the
# missing-binary stderr was swallowed by the || true capture. The other
# stresstests (cli_config, custom_features) already pin release; debug
# stays preferred when present so a dev-workstation run is unchanged.
BIN="./target/debug/cosmostrix"
if [ ! -x "$BIN" ]; then
	BIN="./target/release/cosmostrix"
fi
if [ ! -x "$BIN" ]; then
	echo "ERROR: no cosmostrix binary — build release (cargo build --release) or debug first"
	exit 1
fi
PASS=0
FAIL=0
